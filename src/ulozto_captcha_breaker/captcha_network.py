...
class CaptchaNetwork:
    def __init__(self, image_shape, classes: int, image_preprocess_pipeline, label_preprocess_pipeline, args):
        ...
        input = tf_keras.layers.Input(shape=input_shape)

        layer = input

        if not args.pretrained_model:
            # to normalize input
            layer = tf_keras.layers.BatchNormalization()(layer)
            layer = tf_keras.layers.Convolution2D(
                filters=32, kernel_size=7, strides=2, padding="same", use_bias=False,
                kernel_regularizer=tf_keras.regularizers.l2(args.l2))(layer)
            layer = tf_keras.layers.BatchNormalization()(layer)
            layer = tf_keras.layers.ReLU()(layer)
            layer = tf_keras.layers.MaxPooling2D(strides=2)(layer)
            ...
            layer = tf_keras.layers.GlobalAveragePooling2D()(layer)

            layer = tf_keras.layers.Dense(units=args.captcha_length * classes,
                                          kernel_regularizer=tf_keras.regularizers.l2(args.l2))(layer)
            # # reshape into (batch, letters_count, rest)
            target_shape = (args.captcha_length, classes)
            layer = tf_keras.layers.Reshape(target_shape=target_shape)(layer)

            # layer = tf_keras.layers.Dense(units=100, activation="relu", kernel_regularizer=tf_keras.regularizers.l2(0.01))(layer)
            # layer = tf_keras.layers.Dropout(0.5)(layer)
            output = tf_keras.layers.Dense(units=classes, activation="softmax")(layer)

            self._model = tf_keras.Model(inputs=input, outputs=output)
        else:
            self._model = tf_keras.models.load_model(args.pretrained_model)
        ...
        self._model.compile(optimizer=tf_keras.optimizers.Adam(),
        ...
        self._tb_callback = tf_keras.callbacks.TensorBoard(args.logdir, update_freq=1000, profile_batch=1)
    ...
    def _create_residual_block(self, layer: tf_keras.layers.Layer, filters: int, l2: float):
        prev_layer = layer
        layer = tf_keras.layers.BatchNormalization()(layer)
        layer = tf_keras.layers.ReLU()(layer)
        layer = tf_keras.layers.Convolution2D(
            filters=filters, kernel_size=3, strides=1, padding="same", use_bias=False,
            kernel_regularizer=tf_keras.regularizers.l2(l2))(layer)
        ...
        layer = tf_keras.layers.Convolution2D(
            filters=filters, kernel_size=3, strides=1, padding="same", use_bias=False,
            kernel_regularizer=tf_keras.regularizers.l2(l2))(layer)
        layer = tf_keras.layers.Add()([prev_layer, layer])
